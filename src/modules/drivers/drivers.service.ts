...
export class DriversService {
  ...
  async findAll({ search, status, partnerId, licenseExpiringWithinDays, page, limit, scope }: FindAllParams) {
    ...
    const [rows, total] = await Promise.all([
      this.prisma.driver.findMany({
        ...
        include: { partner: true, assignedVehicle: true },
      }),
  ...
  private async enrich(
    driver: Prisma.DriverGetPayload<{ include: { partner: true; assignedVehicle: true } }>,
  ) {
    const { partner, assignedVehicle, ...rest } = driver;
    return {
      ...rest,
      ...
      assignedVehiclePlate: assignedVehicle?.plateNumber ?? null,
    };
  }
}
