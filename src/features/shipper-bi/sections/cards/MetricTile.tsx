 ...
 *
 * The small quadrant of the control tower. Deliberately not a `KpiCard` — these
 * sit four to a block and have room for a value and one visual cue, so the
 * sparkline is a bare inline shape with no axes rather than a plot with chrome
 * that would not be legible at this size.
 *
 * Label above, figure below: the label is read once to learn what the tile is
 * and never again, the figure is read every time. Putting the label on top and
 * anchoring the figure to the bottom edge also lines all four figures up across
 * the block, which a value-then-caption stack cannot do when the labels wrap to
 * different heights.
 */
...
export interface MetricTileProps {
  ...
  label?: string;
  /** Icon displayed in top left of card with brand color background */
  icon?: ReactNode;
  /** Micro-analysis caption (e.g. "Avg 2.6 / day", "Target 90.0%") */
  subAnalysis?: string;
  ...
  const body: ReactNode = (
    <div className="flex flex-col h-full justify-between gap-2.5">
      ...
      <div className="flex items-center justify-between gap-2">
        {/* Two lines rather than an ellipsis — the solid 36px chip leaves the
            label ~97px, and "On-Time Delivery Rate" clipped to "On-Time Deliv…"
            costs the reader the words that say which rate. */}
