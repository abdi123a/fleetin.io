...
/**
 * Access requests — the self-service queue.
 *
 * "Register" (RegisterPage.tsx) does not create an account: it files a request
 * here, and approving one creates the account in the local directory that
 * login() falls back to — see access-request.store.ts. That directory is
 * separate from the server's user table, which is what the Users tab manages;
 * approving a request is a decision about a person, not a grant of platform
 * permissions.
 */
